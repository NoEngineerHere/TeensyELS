...
void setup() {
  ...
  display.init();
        display.update(
          ...
          lockState, enabled, jogMode, synced);
...
void loop() {
  ...
  directionIncrement = pulseCount >= 0 ? -1 : 1;

  if (driveMode && leadscrewAngleCumulative == 0){
    enabled = false;
  ...
  if (!enabled && pulseCount != 0){ //state 2, keeping track of spindle position
  ...
  if (jogMode) { //state 3, jogging

      jogdirectionIncrement = jogpulseCount >= 0 ? -1 : 1;

      if (jogpulseCount > 0) {
        CW;
      } else if (jogpulseCount < 0) {
        CCW;
      }
      ...
      while(jogpulseCount != 0){
        ...
        int thisJogPulse = micros();

        if (thisJogPulse - lastjogPulse > jogStepTime){

        digitalWriteFast(stp, HIGH);
        delayMicroseconds(2);
        digitalWriteFast(stp, LOW);
        delayMicroseconds(2);
        lastjogPulse = thisJogPulse;
        jogpulseCount += jogdirectionIncrement;
        leadscrewAngleCumulative += jogdirectionIncrement;
